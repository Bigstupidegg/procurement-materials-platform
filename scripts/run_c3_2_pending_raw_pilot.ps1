...
$Pilot = Join-Path $RepositoryRoot "scripts\c3_2_pending_raw_persistence.py"
$CredentialPath = Join-Path $RepositoryRoot $CredentialFile
if (-not (Test-Path -LiteralPath $Pilot)) { Fail "Pending Raw pilot entry point was not found." }
...
$env:ALLOW_PENDING_RAW_WRITE = "1"
$env:GOOGLE_SHEET_ID = $SheetId
$env:GOOGLE_SERVICE_ACCOUNT_FILE = $CredentialPath
$env:PYTHONUNBUFFERED = "1"
$env:PYTHONUTF8 = "1"

Write-Host "C3.2-2 Pending Raw Windows Pilot"
Write-Host "google_sheet_write=DISABLED pending_raw_write=ENABLED mode=PENDING_RAW_ONLY"
...
try {
    if (-not $Process.Start()) { Fail "Pending Raw process did not start." }
    ...
    Fail "Pending Raw process execution failed."
}
...
$LogPath = Join-Path $LogDirectory "c3_2_pending_raw_pilot-$Timestamp.log"
...
try {
    ...
    Write-Host "[FAIL] Pending Raw log write failed."
    exit 1
}
