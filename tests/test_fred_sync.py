...
class FredSyncTests(unittest.TestCase):
    ...
    def test_parse_metadata_monthly(self):
        ...
        self.assertEqual(result["unitsShort"], "USD/mt")

    def test_parse_metadata_rejects_non_monthly(self):
        payload = {
            "seriess": [
                {
                    "id": "TEST",
        ...
        with self.assertRaises(RuntimeError):
            MODULE.parse_series_metadata("TEST", payload)

    def test_parse_observations_skips_missing(self):
        observations = []
    ...
    def test_parse_observations_rejects_duplicate_month(self):
        observations = []
        for year in range(2020, 2025):
            for month in range(1, 13):
                observations.append(
        ...
        observations[-1] = {"date": "2024-11-15", "value": "11"}
        with self.assertRaises(RuntimeError):
            MODULE.parse_observations("TEST", {"observations": observations})
    ...
    def test_build_status_keeps_world_bank_primary(self):
        ...
        result = MODULE.build_status("2026-08-04T00:00:00Z", fred, previous)
        self.assertEqual(result["dataMode"], "WORLD_BANK_PRIMARY")
        self.assertEqual(result["fred"]["role"], "INDEPENDENT_COMPARISON_ONLY")
        self.assertTrue(result["fred"]["apiKeyRequired"])
        self.assertEqual(result["fred"]["downloadMethod"], "FRED_API_JSON")
